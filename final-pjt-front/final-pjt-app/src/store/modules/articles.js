...
export default {
  ...
  getters: {
    ...
    isAuthor: (state, getters) => {
      // state.article.user까지는 undefined로 괜춘! -> ?를 붙여서 username에 대한 오류 해결
      // article에 user 있어? -> 없으면 username 안쳐다보고 끝냄
      return state.article.user?.username === getters.currentUser.username
    },
    // lodash의 비어있는 객체 판별함수를 활용
  ...
  mutations: {
    ...
    SET_SECRET_NUMBER: (state, number) => (state.sn = number),
  },

  actions: {
    fetchArticles({ commit, getters}, number) {
      /* 게시글 목록 받아오기
      ...
      */
      axios({
        url: drf.articles.articles(number),
        method: 'get',
        headers: getters.authHeader,
      })
        .then(res => commit('SET_ARTICLES', res.data))
        // err 까지만 쓰면 error message가 짧음!
    ...
    createArticle({ commit, getters }, article) {
      ...
      */
      console.log(article)
      axios({
        url: drf.articles.articles(article.secretnumber),
        method: 'post',
        data: article,
        headers: getters.authHeader,
      })
    ...
    // 인자는 묶어서 하나로 보내줘야함
    updateArticle({ commit, getters }, { pk, title, content, secretnumber }) {
      /* 게시글 수정
      ...
      */
      axios({
        ...
        method: 'put',
        data: { title, content, secretnumber },
        headers: getters.authHeader,
      })
        .then(res => {
          commit('SET_ARTICLE', res.data)
          // console.log(this.getters.article)
          router.push({
            name: 'article',
            params: { articlePk: getters.article.id }
          })
    ...
    SetSecretNumber({commit}, number){
      commit('SET_SECRET_NUMBER', number)
    }
  },
}