      ...
      <div class="style=width:700px;">
        <table class="table table-hover">
          ...
          <tbody class="table-light">
            <tr v-for="like in paginatedData" :key="like.pk">
              <th>{{ like.pk }}</th>
              <th>
                <router-link :to="{ name: 'article', params: { articlePk : like.pk } }">
                  <p>{{ like.title }}</p>
                </router-link>
              </th>
            </tr>
...
export default {
  ...
  computed: {
    ...mapGetters(['profile',]),
    pageCount(){
        let Length = this.profile.like_articles.length
    ...
    paginatedData(){
      ...
      return this.profile.like_articles.slice(start, end)
    }
  },
  created() {
